...
const LatLongForm = props =>{
    ...
    if (lat !==null && long != null){
        //submit button will be enabled when both the inputs are not null
        button = <Button variant='info' size='lg'  onClick={onSubmitHandler} >Submit</Button>    
    }

    return (
        // rendering user input form for latitude and longitude
        <Form >
            <Form.Row >
...
const mapDispatchToProps = dispatch =>{
    //dispatching on button submit to call store action for fetching images with input coordinates
    return{
